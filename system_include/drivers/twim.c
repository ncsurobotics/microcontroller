 ...
 * \param package   Package information and data
 *                  (see \ref twi_package_t)
 * \param read1      Selects the (primary) transfer direction
 * \param read2      Selects the (secondary) transfer direction
 *
 * \return  status_code_t
 *      - STATUS_OK if the transfer completes
 *      - ERR_BUSY to indicate an unavailable bus
 ...
 *      - ERR_INVALID_ARG to indicate invalid arguments.
 */
status_code_t twi_master_transfer2(TWI_t *twi,
		const twi_package_t *package, bool read1, bool read2)
{
	/* Do a sanity check on the arguments. */
	if ((twi == NULL) || (package == NULL)) {
		return ERR_INVALID_ARG;
	...
	if (STATUS_OK == status) {
		...
		/* if user has specified addr_length or user want to write */
		if (package->addr_length || (false == read1)) {
			transfer.bus->MASTER.ADDR = chip;
